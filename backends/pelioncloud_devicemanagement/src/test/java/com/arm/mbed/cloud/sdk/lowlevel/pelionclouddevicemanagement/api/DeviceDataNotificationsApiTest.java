...
/**
 * API tests for DeviceDataNotificationsApi
 */
public class DeviceDataNotificationsApiTest {

    private DeviceDataNotificationsApi api;

    @Before
    public void setup() {
        api = new ApiClient().createService(DeviceDataNotificationsApi.class);
    }

    /**
     * Open the websocket.
     *
     * A websocket channel can have only one active connection at a time. If a websocket connection for a channel exists
     * and a new connection to the same channel is made, the connection is accepted and the older connection is closed.
     * A websocket client library should be used when connecting to this endpoint. Once the socket has been opened, the
     * server may close it with one of the following status codes. &lt;table&gt; &lt;thead&gt; &lt;tr&gt;
     * &lt;th&gt;Code&lt;/th&gt; &lt;th&gt;Description&lt;/th&gt; &lt;/tr&gt; &lt;/thead&gt; &lt;tbody&gt; &lt;tr&gt;
     * &lt;td&gt;&lt;b&gt;1000&lt;/b&gt;&lt;/td&gt; &lt;td&gt;Socket closed normally by the client, or by the server
     * when the channel is deleted with a REST call.&lt;/td&gt; &lt;/tr&gt; &lt;tr&gt;
     ...
     *
     * Delete a notification websocket channel bound to the application. This is required to change the channel from
     * websocket to another type. Deleting the websocket channel also removes the channel&#39;s notification queue. Any
     * unsent notifications are lost when the channel is deleted. **Example:** &#x60;&#x60;&#x60; curl -X DELETE
     ...
     * Delete callback URL.
     *
     * Deletes the callback URL. Deleting the callback URL also removes the channel&#39;s notification queue. Any unsent
     * notifications are lost when the channel is deleted. **Example:** &#x60;&#x60;&#x60; curl -X DELETE
     ...
     *
     * Returns 200 with websocket connection status, if websocket channel exists. **Note**: The current version does not
     * yet have the capability to determine the channel status and will always return status &#39;unknown&#39; and queue
     * size &#39;-1&#39;. **Example:** &#x60;&#x60;&#x60; curl -X GET
     * https://api.us-east-1.mbedcloud.com/v2/notification/websocket \\ -H &#39;Authorization: Bearer
     * &lt;access_key&gt;&#39; &#x60;&#x60;&#x60;
     */
    @Test
     ...
     * polling connection per application can be ongoing at any given time. We recommend using a persistent connection
     ...
     * **notification callback**. There can only be one notification channel per application in Device Management
     * Connect. If a notification channel of other type already exists for the application, delete it before creating a
     * long poll notification channel. **Example:** &#x60;&#x60;&#x60; curl -X GET
     ...
     * headers are application-specific. One possible use for additional headers is checking the origin of a PUT
     * request, as well as distinguishing the application to which the notification belongs. **Note**: Only one callback
     * URL for each application can be active. If you register a new URL while another one is active, it replaces the
     * active one. There can be only one notification channel at a time for each application. If another type of channel
     * is already present, you need to delete it before setting the callback URL. **Expiration of a callback URL:** A
     * callback can expire when Device Management cannot deliver a notification due to a connection timeout or error
     * response (4xx or 5xx). After each delivery failure, Device Management sets an exponential back-off time and makes
     * a retry attempt after that. The first retry delay is 1 second, then 2s, 4s, 8s, up to a maximum delay of two
     * minutes. The retry delay is applied when the response is received, or in case of timeout, after the timeout
     * expires. The request timeout is 20 seconds; in the case of timeout, the first retry happens 20 + 1 seconds after
     * the first delivery attempt, then 20 + 2 seconds, and so on. The callback URL is removed if all retries fail
     * within 24 hours. More about [notification sending
     ...
     *
     * Register (or update) a channel using websocket connection to deliver notifications. The websocket channel should
     * be opened by client using &#x60;/v2/notification/websocket-connect&#x60; endpoint. To get notifications pushed,
     * you must place subscriptions. For more information on notification messages, see
