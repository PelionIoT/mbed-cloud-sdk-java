...
/**
 * Following tests create a fake server that supplies client with elements. They
 * then verify that the paginator works as intended by making multiple calls to
 * the server
 */
public class TestPaginator {
	...
	public final void testOnePageIteration() {
		...
		FakeServer server = new FakeServer(1, elementNumber);
		try {
	...
	public final void testMultiplePagesIteration() {
		...
		long idSum = 0;
		long valueSum = 0;
		FakeServer server = new FakeServer(pageNumber, elementNumber);
		try {
			Paginator<FakeElement> paginator = new Paginator<>(new PageRequester<FakeElement>() {
