...
public class ModelAdapterFetcher extends ArtifactFetcher<ModelAdapter> {
    ...
    public ModelAdapter fetch(ParameterType fromFieldType, ParameterType toFieldType) {
        ...
        if (fromFieldType.isModel()) {
            ...
            adapter.addDefaultConversion(modelFetcher.fetch(fromFieldType),
                                         toFieldType.isModel() ? modelFetcher.fetch(toFieldType)
        ...
        if (toFieldType.isModel()) {
            ...
            adapter.addDefaultConversion(new Model(fromFieldType.getClazz()), modelFetcher.fetch(toFieldType));
            return adapter;
        }
        return null;
    }
